...
(define (read-atom port c)
  ...
  (define (atomize L)
    (let ((s (list->string (reverse L))))  ;;(make-string len #\space)))
      (or (string->number s)
  ...
  (let loop ((prefix #f) (L (list c)))
    (let ((c (peek-char port)))
      (if (symbol-terminator? c)
          (let ((new-prefix (combine-dotted prefix (atomize L))))
            (cond ((eqv? c #\.)
                   (read-char port)
                   (let ((c (peek-char port)))
                     (if (not (symbol-constituent? c))
                         (error 'read "Symbol ending in '.'"))
                     (loop new-prefix (list (read-char port)))))
                  (else new-prefix)))
          (loop prefix (cons (read-char port) L))))))

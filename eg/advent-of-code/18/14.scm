...
(to (step!)
  ...
  (for each! (((_ i elf) elves.items)) ;maybe a method like array .update!
...
(to (show)
  (for each! (((_ i recipe) board.items))
...
;    (show)
    (begin checking ((j i))
      (if (< (+ j pc) board.count)
          (do (when (for every (((_ k p) pattern.items))   ;(= pattern (board .slice j (+ j pc)))   ; ugh
                      (= p (board (+ j k))))
                (print j)
                (os-exit 0))
              (checking j.+))
